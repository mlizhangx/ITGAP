...
class NegativeSamplingTool:
    ...
    def load_datasets(self) -> ad.AnnData:
        ...
        h5ad_10x = self.data_dir / "merge_gex_all_donors_all_peptides_meta.h5ad"
    ...
    def generate_large_negative_population(self, positive_pairs: pd.DataFrame,
        ...
        n_unique_peptides = len(peptide_instances)
        print(f"    Unique peptides available for sampling: {n_unique_peptides:,}")

        # Cap k at the number of unique peptides — beyond that, further attempts
        # are guaranteed to collide or produce duplicates.
        effective_k = min(k_multiplier, n_unique_peptides)
